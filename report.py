...
# Classes that are correct by-design exclusions, not actionable gaps: process
# plumbing (lifecycle/index), stateful aggregation (a stateless RPC cannot hold
# the accumulator), and the Datum/array-generic workers whose typed variant is
# the exposed surface. These are reason-marked and reported, but excluded from
# the coverage base so the percentage measures the *actionable* surface — the
# signature irregularities upstream uniformization removes. no-codec stays
# counted: some entries are internal node types, others merely lack an in/out
# wrapper, so excluding it wholesale would overstate coverage.
_BY_DESIGN = frozenset({"plumbing", "stateful", "internal-generic"})
...
def build_report(catalog: dict) -> dict:
    ...
    total = len(pub)
    n_exp = len(exposable)
    by_design = sum(n for k, n in by_class.items() if k in _BY_DESIGN)
    # Coverage is over the actionable base: public functions minus the
    # by-design exclusions, which no signature change can turn into a
    # stateless RPC. The full gap (incl. by-design) stays in the worklist.
    base = total - by_design
    return {
        "publicTotal": total,
        "exposable": n_exp,
        "byDesignExcluded": by_design,
        "coveragePct": round(n_exp * 100 / base, 1) if base else 0,
        "internalExcluded": len(fns) - total,
        "gap": total - n_exp,
        "actionableGap": total - n_exp - by_design,
...
def main() -> None:
    ...
    print(f"[coverage] actionable {rep['exposable']}/"
          f"{rep['publicTotal'] - rep['byDesignExcluded']} "
          f"({rep['coveragePct']}%), actionable gap {rep['actionableGap']}, "
          f"{rep['byDesignExcluded']} by-design excluded, "
